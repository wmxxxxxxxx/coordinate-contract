...
func DeployContract() {
	codePath := "contract" // 编译好的二进制文件 go build -o counter
...
func InvokeUploadLawEnforcement(uuid string, domain string, uploader string, upload_time string, hash_abstract string) string{
	account := getAccount()
	...
	args := map[string]string{
		...
		"hash_abstract": hash_abstract,
	}

	var tx *xuper.Transaction
	tx, err = xchainClient.InvokeNativeContract(account, contractName, "UploadLawEnforcement", args)
	if err != nil {
		panic(err)
	}
